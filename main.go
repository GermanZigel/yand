...
func (t Training) distance() float64 {
	var dist float64
	dist = t.Action * t.LenStep / MInKm
	return dist
}

// meanSpeed возвращает среднюю скорость бега или ходьбы.
func (t Training) meanSpeed() float64 {
	// Переводим продолжительность тренировки в секунды
	intervalSeconds := int(t.Duration.Seconds())
	var speed float64
	// Проверяем, чтобы не происходило деления на 0
	if t.LenStep > 0 && intervalSeconds > 0 {
		speed = float64(t.Action*t.LenStep) / float64(intervalSeconds)
	} else {
		// Возвращаем 0 в случае некорректных данных
		speed = 0
	}
	return speed
}
...
func (r Running) Calories() float64 {
	C := ((CaloriesMeanSpeedMultiplier*r.Speed + CaloriesMeanSpeedShift) * r.Weight / MInKm * float64(r.Training.Duration.Hours()) * 60)
	return C
}
...
func (r Running) TrainingInfo() InfoMessage {
	var M InfoMessage
	M.Duration = r.Training.Duration
	M.TrainingType = r.Training.TrainingType
...
func (w Walking) Calories() float64 {
	...
	s := ((CaloriesWeightMultiplier*w.Weight + (w.meanSpeed()*2/w.Height)*CaloriesSpeedHeightMultiplier*w.Height) * float64(w.Training.Duration.Minutes()))
	return s
...
func (w Walking) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var M InfoMessage
	M.Duration = w.Training.Duration
	M.TrainingType = w.Training.TrainingType
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	m := s.LengthPool * s.CountPool / MInKm / float64(s.Training.Duration.Hours())
	return m
}
...
func (s Swimming) Calories() float64 {
	// вставьте ваш код ниже
	c := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Training.Duration.Hours()
	return c
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	var M InfoMessage
	M.Duration = s.Training.Duration
	M.TrainingType = s.Training.TrainingType
...
func ReadData(t CaloriesCalculator) string {
